...
import { AttendanceStatus, DayOfWeek, Gender, Level, Priority, Role, TargetType, } from '../generated/prisma/index.js';
...
function getSchoolDays(count, rng) {
    const days = [];
...
async function createManyInChunks(rows, createMany, chunkSize = 250) {
...
async function main() {
    ...
    const subjects = await Promise.all(subjectsSeed.map((name, index) => prisma.subject.create({
        data: {
            name,
    ...
    const yearGroups = await Promise.all(yearGroupSpecs.map((group) => prisma.yearGroup.create({
        data: {
            ...group,
        },
    })));
    const yearGroupSubjectMap = new Map();
    ...
    for (const group of yearGroups) {
        ...
        const groupTeachers = teachers.filter((teacher) => teacher.taughtYearGroups.some((entry) => entry.id === group.id));
        for (const day of days) {
            for (const period of createdPeriods) {
                ...
                const teacher = subjectTeachers[(group.id + period.id + day.length) % subjectTeachers.length] ??
    ...
    for (const student of students) {
        ...
        for (const fee of groupFees) {
            ...
            const amountPaid = paymentMode < 0.15
                ? 0
                : paymentMode < 0.45
        ...
        data: [
            {
                ...
                content: 'All students and teachers should report to the main hall on Monday for the opening assembly.',
            ...
            },
            {
                ...
                content: 'Class teachers should review attendance records and contact families for repeated absences.',
