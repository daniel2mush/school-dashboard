...
function calculateAttendancePct(record: string[] = []) {
  ...
  const presentish = schoolDays.filter((entry) => entry === "P" || entry === "T").length;
  return Math.round((presentish / schoolDays.length) * 100);
}

function buildDefaultTimetable(yearGroup: any) {
  const subjects = yearGroup.subjects?.length ? yearGroup.subjects : ["Study Hall"];

  return DAYS.reduce((schedule, day, dayIndex) => {
    schedule[day] = PERIOD_OPTIONS.map((_, periodIndex) => subjects[(dayIndex + periodIndex) % subjects.length]);
    return schedule;
  }, {} as Record<string, string[]>);
}

function getInitialFeeStructures(students: any[]) {
  return students.reduce((structures, student) => {
...
function getSubjectOptionsFromState(state: SchoolState) {
  const fromYearGroups = state.yearGroups.flatMap((yearGroup) => yearGroup.subjects || []);
  const fromTeachers = state.teachers.flatMap((teacher) => teacher.subjects || []);
  const fromPublications = state.publications.map((publication) => publication.subject);
  const fromColors = Object.keys(SUBJECT_COLORS);

  return Array.from(new Set([...fromYearGroups, ...fromTeachers, ...fromPublications, ...fromColors])).sort();
}
...
function normalizeState(input: Partial<SchoolState>) {
  const yearGroups = deepClone(input.yearGroups || YEAR_GROUPS).map((yearGroup: any, index: number) => {
    const colorPair = getColorPair(index);
    return {
      ...yearGroup,
      subjects: Array.from(new Set((yearGroup.subjects || []).filter(Boolean))),
  ...
  const attendance = Object.entries(input.attendance || ATTENDANCE).reduce((records, [studentId, record]) => {
    records[String(studentId)] = Array.isArray(record) ? record.slice(-20) : [];
    return records;
  }, {} as Record<string, string[]>);

  const students = deepClone(input.students || STUDENTS).map((student: any) => {
    const record = attendance[String(student.id)] || [];
    const totalFee = feeStructures[String(student.year)] ?? feeStructures[student.year] ?? student.fees.total;
  ...
  const studentCountByYear = students.reduce((counts, student) => {
  ...
  const timetable = normalizedYearGroups.reduce((schedule, yearGroup) => {
    schedule[String(yearGroup.id)] = input.timetable?.[yearGroup.id] || input.timetable?.[String(yearGroup.id)] || buildDefaultTimetable(yearGroup);
    return schedule;
  }, {} as Record<string, Record<string, string[]>>);

  return {
    school: deepClone(input.school || SCHOOL),
...
export function SchoolDataProvider({ children }: { children: ReactNode }) {
  ...
  const saveYearGroup = (input: any) => {
    updateState((current) => {
      const existingIndex = current.yearGroups.findIndex((yearGroup) => yearGroup.id === input.id);
      ...
      if (existingIndex >= 0) {
        ...
        return {
          ...
          feeStructures: {
            ...
            [input.id]: Number(input.feeTotal) || current.feeStructures[input.id] || current.feeStructures[String(input.id)] || 3000,
          },
        };
      }
      ...
      teachers: current.teachers.map((teacher) => {
        ...
        return {
          ...
          years: shouldHaveYear
            ? Array.from(new Set([...teacher.years, yearId])).sort((a, b) => a - b)
            : teacher.years.filter((assignedYearId) => assignedYearId !== yearId),
        };
      }),
    }));
  };
  ...
  const addPublication = ({ name, year, subject, published, teacherId }: any) => {
    updateState((current) => ({
      ...current,
      publications: [
        {
  ...
  const saveAttendance = ({ statusesByStudent }: any) => {
    updateState((current) => {
      ...
      Object.entries(statusesByStudent).forEach(([studentId, status]) => {
        ...
        nextAttendance[String(studentId)] = [...existingRecord, String(status).charAt(0).toUpperCase()].slice(-20);
      });
  ...
  const saveTimetableSlot = ({ yearId, day, periodIndex, subject }: any) => {
    updateState((current) => {
      const yearSchedule = current.timetable[String(yearId)] || buildDefaultTimetable(current.yearGroups.find((yearGroup) => yearGroup.id === yearId));
      const nextDay = [...(yearSchedule[day] || Array.from({ length: 5 }, () => "-"))];
  ...
  const saveGrade = ({ studentId, subject, midTerm, assignmentAvg, teacher, comment }: any) => {
    updateState((current) => {
      const score = Math.round(Number(midTerm) * 0.6 + Number(assignmentAvg) * 0.4);
      const nextEntry = {
        subject,
        score,
      ...
      const existingIndex = existingGrades.findIndex((grade) => grade.subject === subject);

      const nextGrades =
        existingIndex >= 0
          ? existingGrades.map((grade, index) => (index === existingIndex ? nextEntry : grade))
  ...
  return <SchoolDataContext.Provider value={contextValue}>{children}</SchoolDataContext.Provider>;
}
