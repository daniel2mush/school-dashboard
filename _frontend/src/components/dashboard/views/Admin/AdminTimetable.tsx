...
function TimetableEditModal({
  ...
  const periods = useMemo(() => getPeriods(editState.yearGroup), [editState.yearGroup]);
        ...
        <div className={styles.modalGrid}>
          ...
          <label className={styles.field}>
            ...
            <select
              ...
              disabled={!subjectId}
            >
              <option value="">{subjectId ? "Teacher unassigned" : "Select subject first"}</option>
      ...
      >
        ...
        <div className={styles.addPeriodForm}>
          ...
          <button
            ...
            onClick={handleAdd}
            disabled={isCreating || isAssigning || !newLabel || !newStart || !newEnd}
          >
            <Plus size={16} />
            {isCreating || isAssigning ? "Adding..." : "Add Period to Year Group"}
          </button>
        </div>
...
export default function AdminTimetable() {
  ...
  const [managePeriodsYearGroup, setManagePeriodsYearGroup] = useState<AdminYearGroupStructure | null>(null);
  ...
  const totalScheduledLessons = yearGroups.reduce(
    (count, yearGroup) =>
      count +
      yearGroup.timetables.filter((slot) => slot.subject?.name).length,
    0,
  );
    ...
    <section className={styles.view}>
      <header className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <h2 className={styles.title}>A clearer view of the master timetable</h2>
      ...
      {editState ? (
        <TimetableEditModal editState={editState} onClose={() => setEditState(null)} />
      ) : null}
