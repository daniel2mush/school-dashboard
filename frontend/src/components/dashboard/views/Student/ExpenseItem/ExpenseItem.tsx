    ...
    <div className={`${styles.item} ${item.isFullyPaid ? styles.itemPaid : ''}`}>
      ...
      <div className={styles.header}>
        ...
        <div className={`${styles.badge} ${item.isFullyPaid ? styles.badgePaid : styles.badgePending}`}>
          {item.isFullyPaid ? (
            <><CheckCircle size={12} /> {t('student.fees.fullyPaid')}</>
          ) : (
            <><AlertCircle size={12} /> {t('student.fees.pending')}</>
          )}
        </div>
      </div>
      ...
      <div className={styles.grid}>
        <GridCell
          label={t('student.fees.total')}
          value={formatCurrency(item.amount)}
        />
        <GridCell
          label={t('student.fees.paid')}
          value={formatCurrency(item.paid)}
        />
        <GridCell
          label={t('student.fees.remaining')}
      ...
      {item.amountInWords && (
        <div className={styles.amountInWords}>
          <strong>{t('student.fees.amountInWords')}:</strong>{' '}
          {item.amountInWords}
        </div>
      )}
    </div>
