...
function FeePaymentModal({
  ...
  const [drafts, setDrafts] = useState<Record<number, { amountPaid: string; amountInWords: string; isFullyPaid: boolean }>>(
    Object.fromEntries(
      yearGroup.students.map((student) => {
        const payment = fee.payments.find((entry) => entry.studentId === student.id);
        return [
          student.id,
          {
        ...
        <div className={styles.modalBody}>
          <div className={styles.paymentList}>
            {yearGroup.students.map((student) => {
              ...
              const amountPaid = Number(String(draft.amountPaid).replace(/,/g, "")) || 0;
              ...
              return (
                <article key={student.id} className={styles.paymentRow}>
                  <div className={styles.paymentHeader}>
                    <div>
                      <div className={styles.paymentStudent}>{student.name}</div>
                      <div className={styles.paymentMeta}>{student.email}</div>
                    </div>
                    <Badge variant={fullyPaid ? "green" : remaining > 0 ? "amber" : "gray"}>
                  ...
                  <div className={styles.paymentActions}>
                    ...
                    {payment?.updatedAt ? (
                      <span className={styles.paymentMeta}>
                        Updated {new Date(payment.updatedAt).toLocaleDateString("en-GB")}
                      </span>
                    ) : null}
                  </div>
                </article>
...
export default function AdminFees() {
  ...
  const activeYearGroup =
    yearGroups.find((yearGroup) => yearGroup.id === selectedYearGroupId) || yearGroups[0];
  const activeFee =
  ...
  if (analyticsLoading || feeLoading || !analytics) {
    return <div className={styles.view}>Loading fee workspace…</div>;
  }
        ...
        <div className={styles.heroInner}>
          <div className={styles.heroCopy}>
            ...
            <p className={styles.copy}>
              Set up fee items for each year group, track partial payments student by student,
              and keep a clear view of what has been paid, what is still outstanding, and which
              expenses are fully settled.
            </p>
          </div>
          <div className={styles.heroMeta}>
            ...
            <span className={styles.metaChip}>
              ...
              {analytics.studentsWithOutstandingFees} students still owe at least one item
            </span>
          </div>
        </div>
      </header>
      ...
      <div className={styles.workspace}>
          ...
          <div className={styles.yearGroupList}>
            {yearGroups.map((yearGroup) => {
              ...
              const collected = yearGroup.fees.reduce(
                (sum, fee) =>
                  sum +
                  fee.payments.reduce((paymentSum, payment) => paymentSum + payment.amountPaid, 0),
                0,
              );
        ...
        <div className={styles.mainPanel}>
          {activeYearGroup ? (
            <>
              <div className={styles.panelHeader}>
                <div>
                  ...
                  <p>
                    Add expense lines that apply to every student in this year group, then record
                    payment against each item individually.
                  </p>
                </div>
              </div>
              ...
              <section className={styles.createCard}>
                ...
                <div className={styles.createActions}>
                  <button
                    ...
                    disabled={createFee.isPending || !title.trim() || !amount.trim()}
                  >
                ...
                {activeYearGroup.fees.length === 0 ? (
                  ...
                  <div className={styles.feeList}>
                    {activeYearGroup.fees.map((fee) => {
                      ...
                      const expectedTotal = fee.amount * activeYearGroup.students.length;
                      const paidStudents = fee.payments.filter(
                        (payment) => payment.isFullyPaid || payment.amountPaid >= fee.amount,
                      ).length;
                      ...
                      return (
                        <article key={fee.id} className={styles.feeCard}>
                          <div className={styles.feeCardTop}>
                            <div>
                              <h4>{fee.title}</h4>
                              <p>{fee.description || "Applies to every student in this year group."}</p>
                            </div>
                            <button
                              type="button"
                          ...
                          <div className={styles.feeMetrics}>
                            ...
                            <div>
                              ...
                              <strong>{paidStudents}/{activeYearGroup.students.length}</strong>
                            </div>
                          </div>
            ...
            <div className={styles.emptyState}>No year groups available yet.</div>
          )}
        </div>
      </div>
