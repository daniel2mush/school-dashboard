...
function targetLabel(ann: Announcement, t: (key: string) => string) {
  if (ann.targetType === 'ALL') return t('admin.overview.everyone')
  if (ann.targetType === 'TEACHERS_ONLY')
    return t('admin.overview.teachersOnly')
  return t('admin.overview.yearGroupTarget')
}
    ...
    <section className={styles.view}>
      ...
      <div className={styles.mainGrid}>
        <Card style={{ marginBottom: 0 }}>
          <CardHeader
            title={t('admin.overview.yearGroupsGlance')}
            action={t('admin.overview.manageAll')}
            onAction={() => navigate({ to: '/dashboard/admin/yeargroups' })}
          />
          {!structure?.length ? (
            <div className={styles.emptyState}>
              {t('admin.overview.noYearGroups')}
                    ...
                    <div className={styles.ygMain}>
                      <div className={styles.ygTop}>
                        ...
                        <span className={styles.levelBadge}>
                          {t(
                            `admin.yearGroups.modals.levelOptions.${yearGroup.level}`,
                          )}
                        </span>
                      </div>
                      <div className={styles.ygStats}>
                        <span className={styles.ygStat}>
                          ...
                          {t('admin.overview.studentsCount').replace(
                            '{count}',
                            String(studentCount),
                          )}
                        </span>
                        <span className={styles.ygStat}>
                          ...
                          {t('admin.overview.subjectsCount').replace(
                            '{count}',
                            String(yearGroup.subjects.length),
                          )}
                        </span>
                        <span className={styles.ygStat}>
                          <GraduationCap size={12} strokeWidth={2} />
                          {t('admin.overview.teachersCount').replace(
                            '{count}',
                            String(teacherCount),
                          )}
                        </span>
                      ...
                      <div className={styles.ygProgressLabel}>
                        <span>
                          {capacityValue
                            ? t('admin.overview.capacityVsStudents').replace(
                                '{count}',
                                String(capacityValue),
                              )
                            : t('admin.overview.capacityNotSet')}
                        </span>
                        <span>
        ...
        <div className={styles.sideStack}>
          <Card style={{ marginBottom: 0 }}>
            <CardHeader
              title={t('admin.overview.recentAnnouncements')}
              action={t('admin.overview.openHub')}
              onAction={() =>
            ...
            {announcements.length === 0 ? (
              <div className={styles.emptyState}>
                {t('admin.overview.noAnnouncements')}
                <div className={styles.emptyHint}>
          ...
          <Card style={{ marginBottom: 0 }}>
            <CardHeader title={t('admin.overview.operationalWatchlist')} />
            <div className={styles.watchList}>
              ...
              <div className={styles.watchRow}>
                ...
                <div className={styles.watchBody}>
                  <div className={styles.watchTitle}>
                    {t('admin.overview.teacherCoverage')}
                  </div>
                  <p className={styles.watchText}>
                    {understaffedYears.length > 0
                      ? t(
                          understaffedYears.length === 1
                            ? 'admin.overview.teacherCoverageIssueOne'
                            : 'admin.overview.teacherCoverageIssueMany',
                        ).replace(
                          '{names}',
                          understaffedYears.map((y) => y.name).join(', '),
                        )
                      : t('admin.overview.teacherCoverageClear')}
                  </p>
                </div>
              </div>
              <div className={styles.watchRow}>
                ...
                <div className={styles.watchBody}>
                  <div className={styles.watchTitle}>
                    {t('admin.overview.learningContent')}
                  </div>
                  <p className={styles.watchText}>
                    {t('admin.overview.learningContentCopy')}
                  </p>
                </div>
              </div>
            </div>
