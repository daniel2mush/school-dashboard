...
import { Subject, YearGroup, User } from "@/types/Types";
...
export const useGetTeacherClasses = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        throw new Error(
          responseData.message || responseData.error || "Failed to fetch teacher classes"
        );
      }
...
export const useSubmitGrade = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (data: { studentId: number; subjectId: number; score: number; grade: string }) => {
      ...
      if (!res.ok) {
        throw new Error(
          responseData.message || responseData.error || "Failed to submit grade"
        );
      }
      return responseData;
    },
...
export const useSubmitAttendance = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (data: { studentId: number; status: string; date?: Date }) => {
      ...
      if (!res.ok) {
        throw new Error(
          responseData.message || responseData.error || "Failed to mark attendance"
        );
      }
      return responseData;
    },
